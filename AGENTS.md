...
- `APPLE_CORE_AI_CAPABILITIES.md` records verified public capabilities; `coreai.md` contains notes from the local SDK interfaces.
- `PRODUCT.md` and `GRAND_PLAN.md` describe direction and unfinished milestones. Do not present planned work as implemented.
- `project.yml` is the XcodeGen source of truth. Do not hand-edit `CoreAIFrameworkLab.xcodeproj/project.pbxproj`. After changing targets, packages, settings, or adding/removing/renaming source or resource files, run `xcodegen generate` and commit the resulting project changes.
- `CoreAILab/Resources/AppleModels/apple-coreai-models.json` is generated by `Scripts/update_apple_model_catalog.py`. Keep its source revision aligned with the pinned `coreai-models` revision in `project.yml` and related documentation.
...
Generate the project after project-structure changes:
